# Resource Azure Load balancer inbound nat rule
resource "azurerm_lb_nat_rule" "web_lb_nat_rule_22" {
  depends_on = [ azurerm_linux_virtual_machine.web_linuxvm ]
  count = var.web_linuxvm_instance_count
  name = "var-${count.index}-ssh-${var.lb_inbound_nat_ports[count.index]}-vm-22"
  protocol = "Tcp"
...
## Associate a NAT rule and VM network interface
resource "azurerm_network_interface_nat_rule_association" "web_nic_nat_rule_associate" {
  ...
  nat_rule_id = element(azurerm_lb_nat_rule.web_lb_nat_rule_22[*].id, count.index)
}